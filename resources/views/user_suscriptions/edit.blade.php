<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('messages.edit_suscription') }}
        </h2>
    <link href="https://cdnjs.cloudflare.com/ajax/libs/tailwindcss/2.2.19/tailwind.min.css" rel="stylesheet">
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>{{ __('messages.edit_suscription') }}</title>
    </x-slot>

    @section('content')
    <div class="container mx-auto mt-5  flex-col space-y-4 items-center">
        <h1 class="text-2xl font-bold mb-4 flex flex-col items-center">{{ __('messages.edit_suscription') }}</h1>
        <form action="{{ route('suscriptions.update', $suscription->id) }}" method="POST" class="max-w-lg mx-auto mt-5">
            @csrf
            @method('PUT')
            ...
            <div class="mb-6">
                ...
                <select id="site_id" name="site_id" class="form-select block w-full px-3 py-2 border rounded-md shadow-sm focus:outline-none focus:border-blue-500 focus:shadow-outline-blue @error('site_id') border-red-500 @enderror" required>
                    <option value="" disabled selected>{{ __('messages.currently') }}: {{ $suscription->site->slug }}</option>
            ...
            <div class="mb-6">
                ...
                <input value="{{ old('name', $suscription->name) }}" type="text" id="name" name="name" value="{{ old('name') }}" class="form-input block w-full px-3 py-2 border rounded-md shadow-sm focus:outline-none focus:border-blue-500 focus:shadow-outline-blue @error('name') border-red-500 @enderror" required>
                @error('name')
            ...
            <div class="mb-6">
                ...
                <textarea id="description" name="description" value="{{ old('description') }}" rows="4" cols="50" class="form-input block w-full px-3 py-2 border rounded-md shadow-sm focus:outline-none focus:border-blue-500 focus:shadow-outline-blue @error('description') border-red-500 @enderror" required>{{ $suscription->description }}</textarea>
                @error('description')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-6">
                ...
                <select id="currency" name="currency" class="form-select block w-full px-3 py-2 border rounded-md shadow-sm focus:outline-none focus:border-blue-500 focus:shadow-outline-blue @error('currency') border-red-500 @enderror" required>
                    <option value="" disabled selected>{{ __('messages.currently') }}: {{ $suscription->currency_type }}</option>
            ...
            <div class="mb-6">
                ...
                <input type="number" id="amount" name="amount" value="{{ old('amount', $suscription->amount) }}" class="form-input block w-full px-3 py-2 border rounded-md shadow-sm focus:outline-none focus:border-blue-500 focus:shadow-outline-blue @error('amount') border-red-500 @enderror" required>
                @error('amount')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-6">
                ...
                <select id="frecuency_collection" name="frecuency_collection" class="form-select block w-full px-3 py-2 border rounded-md shadow-sm focus:outline-none focus:border-blue-500 focus:shadow-outline-blue @error('frecuency_collection') border-red-500 @enderror" required>
                    <option value="" disabled selected>{{ __('messages.currently') }}: {{ $suscription->frecuency_collection }}</option>
